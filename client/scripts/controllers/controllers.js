app.controller('siteCtrl', ['$scope', function($scope, site){
  $scope.backgroundImg = 'public/img/oso.JPG';

  $scope.changeBackImg = function(image){

    var newImg = image;
    $scope.backgroundImg = newImg;
...
app.controller('slideCtrl', ['$scope', 'slideService', '$http', '$state', function($scope, slideService, $http, $state){
  $scope.slideForm = {};
  ...
  slideService.retrieve(function(data){
  $scope.slides = data;
  
});
  $scope.createSlide = function(){
   
    var newSlide = new slideService();
    formData.append('title', $scope.slideForm.title);
    formData.append('content', $scope.slideForm.content);
    formData.append('upl', $scope.myFile);
    
    $http.post('/api/event/slides', formData, {
      transformRequest: angular.identity,
      headers: {'Content-Type': undefined}
    }).success(function(){
      alert('data was saved');
    }).error(function(){
      alert('an error has occured and dat was not saved');
    });
  ...
  $scope.deleteSlide = function(sl){
  slideService.remove({id: sl});
  slideService.query(function(data){
    $scope.slides = data;
  });
};
